...
### /quest
Конечно же квест. Последовательность вопросов с проверкой ответа, в которую доступ происходит по логину и паролю команды. Логин и пароль команды, вопросы и ответы к ним записаны в отдельном файле, чтобы их легче было менять. [a relative link](quest.txt)
>/quest
...
## Про установку
Скачиваем репозиторий и запускаем EchoBot.py (да, название с начала разработки я так и не поменял), в остальном проблем возникнуть не должно.
